...
export async function POST(request:Request){
 try{
  ...
  const moods=await db(`daily_moods?student_id=eq.${student.id}&class_id=eq.${student.class_id}&select=id,external_id,emoji_a,emoji_b,note,activity_date,created_at,privacy_level`);
