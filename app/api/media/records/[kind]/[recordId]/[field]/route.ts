...
export async function GET(_:Request,{params}:{params:Promise<{kind:string;recordId:string;field:string}>}){
 try{
  ...
  const config:any={friend:["daily_friend_cards",null],nature:["nature_cards",null],capture:["personal_treasures","privacy_level"],discovery:["discoveries","privacy_level"]},entry=config[kind];if(!entry)return NextResponse.json({error:"missing"},{status:404});
  ...
  if(student){const allowed=kind==="friend"?row.recipient_student_id===student.student.id||row.sender_student_id===student.student.id:kind==="nature"?row.recipient_student_id===student.student.id||row.sender_student_id===student.student.id:row.student_id===student.student.id;if(!allowed||row.class_id!==student.student.class_id)return NextResponse.json({error:"forbidden"},{status:403})}
